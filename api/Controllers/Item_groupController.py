...
def read_item_group(item_group_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
...
def create_item_group(item_group: dict, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
    existingitem_group = data_provider.fetch_item_group_pool().get_item_group(item_group["id"])
...
def update_item_group(item_group_id: int, item_group: dict, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()    
    existingitem_group = data_provider.fetch_item_group_pool().get_item_group(item_group_id)
...
def delete_item_group(item_group_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
