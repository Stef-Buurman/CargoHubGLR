...
def read_item_type(item_type_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
...
def read_items_for_item_type(item_type_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
    item_type = data_provider.fetch_item_type_pool().get_item_type(item_type_id)
    if item_type is None:
        raise HTTPException(status_code=404, detail=f"Item_type with id {item_type_id} not found")
...
def create_item_type(item_type: dict, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
    existingitem_type = data_provider.fetch_item_type_pool().get_item_type(item_type["id"])
...
def update_item_type(item_type_id: int, item_type: dict, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()    
...
def delete_item_type(item_type_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
