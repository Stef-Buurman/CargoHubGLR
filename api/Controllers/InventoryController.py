...
def read_inventory(inventory_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
...
def create_inventory(inventorie: dict, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
    existingInventorie = data_provider.fetch_inventory_pool().get_inventory(inventorie["id"])
...
def delete_inventory(inventory_id: int, api_key: str = Depends(auth_provider.get_api_key)):
    data_provider.init()
