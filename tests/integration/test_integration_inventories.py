...
def test_add_inventory_no_api_key(client):
    ...
    assert response.status_code == 403
    responseGet = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
...
def test_get_inventory_by_id(client):
    response = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response.status_code == 200
...
def test_get_inventory_invalid_api_key(client):
    response = client.get('/inventories/' + str(test_inventory['id']), headers=invalid_headers)
    assert response.status_code == 403 
...
def test_update_inventory_no_api_key(client):
    ...
    response = client.put('/inventories/' + str(test_inventory['id']), json=updated_inventory)
    assert response.status_code == 403
    response_get_inventory = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response_get_inventory.status_code == 200
    assert response_get_inventory.json()['total_available'] == test_inventory['total_available']


def test_update_inventory_invalid_api_key(client):
    updated_inventory = test_inventory.copy()
    updated_inventory['total_available'] = test_inventory['total_available'] + 100
    response = client.put('/inventories/' + str(test_inventory['id']), json=updated_inventory, headers=invalid_headers)
    assert response.status_code == 403
    response_get_inventory = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response_get_inventory.status_code == 200
    assert response_get_inventory.json()['total_available'] == test_inventory['total_available']
...
def test_update_invalid_inventory_id(client):
    updated_inventory = test_inventory.copy()
    updated_inventory['total_available'] = test_inventory['total_available'] + 100
    response = client.put('/inventories/invalid_id', json=updated_inventory, headers=test_headers)
    assert response.status_code == 422
...
def test_update_inventory(client):
    ...
    response = client.put('/inventories/' + str(test_inventory['id']), json=updated_inventory, headers=test_headers)
    assert response.status_code == 200
    response_get_inventory = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response_get_inventory.status_code == 200
    assert response_get_inventory.json()['total_available'] == updated_inventory['total_available']
...
def test_delete_inventory_invalid_api_key(client):
    response = client.delete('/inventories/' + str(test_inventory['id']), headers=invalid_headers)
    assert response.status_code == 403
    response_get_inventory = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response_get_inventory.status_code == 200
...
def test_delete_inventory(client):
    response = client.delete('/inventories/' + str(test_inventory['id']), headers=test_headers)
    assert response.status_code == 200
    response_get_inventory = client.get('/inventories/' + str(test_inventory['id']), headers=test_headers)
