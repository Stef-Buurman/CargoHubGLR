...
def test_get_items_for_item_type_non_existing_id(client):
    response = client.get('/item_types/' + str(non_existent_id) + '/items', headers=test_headers)
...
def test_get_items_for_item_type(client):
    ...
    responseDeleteItem = client.delete('/items/' + test_item['uid'], headers=test_headers)
...
def test_update_item_type_no_api_key(client):
    ...
    response = client.put('/item_types/' + str(test_item_type["id"]), json=test_item_type_copy)
    assert response.status_code == 403
    responseGet = client.get('/item_types/' + str(test_item_type["id"]), headers=test_headers)
    assert responseGet.status_code == 200
...
def test_update_item_type_invalid_api_key(client):
    test_item_type_copy = test_item_type.copy()
    test_item_type_copy['name'] = 'Updated Inc'
    response = client.put('/item_types/' + str(test_item_type['id']), json=test_item_type_copy, headers=invalid_headers)
    assert response.status_code == 403
    responseGet = client.get('/item_types/' + str(test_item_type['id']), headers=test_headers)
    assert responseGet.status_code == 200
    assert responseGet.json()["name"] == test_item_type["name"]
...
def test_update_item_type_invalid_id(client):
    test_item_type_copy = test_item_type.copy()
    test_item_type_copy['name'] = 'Updated Inc'
    response = client.put('/item_types/invalidId', json=test_item_type_copy, headers=test_headers)
    assert response.status_code == 422
...
def test_update_item_type_not_found(client):
    test_item_type_copy = test_item_type.copy()
    test_item_type_copy['name'] = 'Updated Inc'
    response = client.put('/item_types/' + str(non_existent_id), json=test_item_type_copy, headers=test_headers)
    assert response.status_code == 404
...
def test_update_item_type(client):
    test_item_type_copy = test_item_type.copy()
    test_item_type_copy['name'] = 'Updated Inc'
    response = client.put('/item_types/' + str(test_item_type['id']), json=test_item_type_copy, headers=test_headers)
    assert response.status_code == 200
    responseGet = client.get('/item_types/' + str(test_item_type['id']), headers=test_headers)
    assert responseGet.status_code == 200
...
def test_delete_item_type_invalid_api_key(client):
    response = client.delete('/item_types/' + str(test_item_type['id']), headers=invalid_headers)
    assert response.status_code == 403
    responseGet = client.get('/item_types/' + str(test_item_type['id']), headers=test_headers)
    assert responseGet.status_code == 200
...
def test_delete_item_type(client):
    response = client.delete('/item_types/' + str(test_item_type['id']), headers=test_headers)
    assert response.status_code == 200
    responseGet = client.get('/item_types/' + str(test_item_type['id']), headers=test_headers)
    assert responseGet.status_code == 404