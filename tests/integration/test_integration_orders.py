...
def test_update_order_no_api_key(client):
    ...
    assert response.status_code == 403
    response_get_order = client.get('/orders/' + str(updated_order['id']), headers=test_headers)
...
def test_update_order_invalid_api_key(client):
    updated_order = test_order.copy()
    updated_order['total_amount'] = test_order['total_amount'] + 100
    response = client.put('/orders/' + str(updated_order['id']), json=updated_order, headers=invalid_headers)
    assert response.status_code == 403
    response_get_order = client.get('/orders/' + str(updated_order['id']), headers=test_headers)
    assert response_get_order.status_code == 200
    assert response_get_order.json()['total_amount'] == test_order['total_amount']
...
def test_update_order_invalid_id(client):
    updated_order = test_order.copy()
    updated_order['total_amount'] = test_order['total_amount'] + 100
    response = client.put('/orders/invalid_id', json=updated_order, headers=test_headers)
    assert response.status_code == 422
...
def test_update_order_non_existent_id(client):
    updated_order = test_order.copy()
    updated_order['total_amount'] = test_order['total_amount'] + 100
    response = client.put('/orders/' + str(non_existent_id), json=updated_order, headers=test_headers)
    assert response.status_code == 404
...
def test_update_order(client):
    updated_order = test_order.copy()
    updated_order['total_amount'] = test_order['total_amount'] + 100
    response = client.put('/orders/' + str(updated_order['id']), json=updated_order, headers=test_headers)
    assert response.status_code == 200
    response_get_order = client.get('/orders/' + str(updated_order['id']), headers=test_headers)
    assert response_get_order.status_code == 200
...
def test_update_order_items_no_api_key(client):
    ...
    for i in range(len(response_get_order.json())):
        assert response_get_order.json()[i - 1]['item_id'] == test_order['items'][i - 1]['item_id']
        assert response_get_order.json()[i - 1]['amount'] == test_order['items'][i - 1]['amount']
...
def test_update_order_items_invalid_id(client):
    test_order_items = test_order['items'].copy()
    test_order_items.append({'item_id': 'P010672', 'amount': 19})
    response = client.put('/orders/invalid_id/items', json=test_order_items, headers=test_headers)
    assert response.status_code == 422
...
def test_update_order_items_non_existent_id(client):
    test_order_items = test_order['items'].copy()
    test_order_items.append({'item_id': 'P010672', 'amount': 19})
    response = client.put(f'/orders/{str(non_existent_id)}/items', json=test_order_items, headers=test_headers)
    assert response.status_code == 404
...
def test_update_order_items(client):
    ...
    for i in range(len(response_get_order.json())):
        assert response_get_order.json()[i - 1]['item_id'] == test_order_items[i - 1]['item_id']
        assert response_get_order.json()[i - 1]['amount'] == test_order_items[i - 1]['amount']


def test_delete_order_no_api_key(client):
    response = client.delete('/orders/' + str(test_order['id']))
    assert response.status_code == 403
    response_get_order = client.get('/orders/' + str(test_order['id']), headers=test_headers)
    assert response_get_order.status_code == 200


def test_delete_order_invalid_api_key(client):
    response = client.delete('/orders/' + str(test_order['id']), headers=invalid_headers)
    assert response.status_code == 403
    response_get_order = client.get('/orders/' + str(test_order['id']), headers=test_headers)
    assert response_get_order.status_code == 200
