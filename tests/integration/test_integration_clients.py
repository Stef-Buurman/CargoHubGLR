...
def test_update_CargoClient_no_api_key(client):
    ...
    response = client.put(f"/clients/{test_CargoClient['id']}", json=updated_CargoClient)
    assert response.status_code == 403
    response_get_CargoClient = client.get(f"/clients/{test_CargoClient['id']}", headers=test_headers)
...
def test_update_CargoClient_invalid_api_key(client):
    updated_CargoClient = test_CargoClient.copy()
    updated_CargoClient['name'] = 'Updated Inc'
    response = client.put(f"/clients/{test_CargoClient['id']}", json=updated_CargoClient, headers=invalid_headers)
    assert response.status_code == 403
    response_get_CargoClient = client.get(f"/clients/{test_CargoClient['id']}", headers=test_headers)
    assert response_get_CargoClient.status_code == 200
    assert response_get_CargoClient.json()['name'] == test_CargoClient['name']
...
def test_update_CargoClient(client):
    updated_CargoClient = test_CargoClient.copy()
    updated_CargoClient['name'] = 'Updated Inc'
    response = client.put(f"/clients/{test_CargoClient['id']}", json=updated_CargoClient, headers=test_headers)
    assert response.status_code == 200
    response_get_CargoClient = client.get(f"/clients/{test_CargoClient['id']}", headers=test_headers)
    assert response_get_CargoClient.status_code == 200
