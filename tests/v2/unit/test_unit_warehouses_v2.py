...
def test_get_warehouse_from_db(warehouse_service, mock_db_service):
    mock_db_service.get.return_value = Warehouse(
        id=4,
        code="VCKINLLK",
        name="Naaldwijk distribution hub",
        address="Izesteeg 807",
    ...
    warehouse = warehouse_service.get_warehouse(4)

    assert warehouse.id == 4
    assert mock_db_service.get.call_count == 1


def test_add_warehouse(warehouse_service, mock_db_service):
    new_warehouse = Warehouse(
        code="VCKINssssssssLLK",
        name="Naaldwijsssssssssssssk distribution hub",
        address="Izesssssssssteeg 807",
        zip="1636 KI",
        city="Naaldwijk",
        province="Utrecht",
        country="NL",
    ...
    mock_db_service.insert.return_value = new_warehouse

    result = warehouse_service.add_warehouse(new_warehouse)

    assert mock_db_service.insert.call_count == 1
    assert new_warehouse in warehouse_service.data
    assert result.name == new_warehouse.name
...
def test_update_warehouse(warehouse_service, mock_db_service):
    ...
    updated_warehouse = warehouse.copy(update={"name": "Updated Warehouse"})
    mock_db_service.update.return_value = updated_warehouse

    result = warehouse_service.update_warehouse(warehouse_id, updated_warehouse)

    assert mock_db_service.update.call_count == 1
    assert result.name == "Updated Warehouse"
    updated_warehouse = warehouse_service.get_warehouse(warehouse_id)
...
def test_update_warehouse_archived(warehouse_service, mock_db_service):
    ...
    updatedwarehouse = warehouse_service.archive_warehouse(warehouse_id)
    assert updatedwarehouse.is_archived == True
    assert mock_db_service.update.call_count == 1

    updated_warehouse = warehouse.copy(update={"name": "Updated Name"})
    ...
    updated_warehouse = warehouse_service.get_warehouse(warehouse_id)
    assert updated_warehouse.is_archived
...
def test_archive_warehouse(warehouse_service, mock_db_service):
    ...
    new_warehouse = warehouse_service.get_warehouse(warehouse_id)
    assert new_warehouse.is_archived


def test_archive_warehouse_not_found(warehouse_service, mock_db_service):
    warehouse_id = non_existent_id
    result = warehouse_service.archive_warehouse(warehouse_id)
...
def test_unarchive_warehouse(warehouse_service, mock_db_service):
    ...
    new_warehouse = warehouse_service.get_warehouse(warehouse_id)
    assert not new_warehouse.is_archived
